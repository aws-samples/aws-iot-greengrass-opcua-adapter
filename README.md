...
2. Configure the server and monitored nodes
   ...
   **Note:**
   + For the node information output from `Advantech Edgelink`, we provide an node converter tool to convert it to `published_nodes.json`, and the following is the example:
      + [nodeFileParser.py](./tool/nodeFileParser.py) : The tool to convert the node information file into published_nodes.json.
      + [sample.csv](./tool/sample.csv) : The node information file generated by Advantech Edgelink.
      + Commands:
        ```
